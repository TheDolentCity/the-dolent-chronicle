...
export function TagList({
	...
	return (
		<ul className={clsx("not-prose flex flex-wrap gap-2 mt-12", className)}>
			{tags.map((tag) => (
...
function TagListItemText({ children }: { children: React.ReactNode }) {
	return <span className="text-xs capitalize leading-none">{children}</span>;
}
